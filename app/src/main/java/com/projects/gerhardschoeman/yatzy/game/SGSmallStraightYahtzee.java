 ...
 */
public class SGSmallStraightYahtzee extends ScoreGroup {
    @Override
    ...
    public ScoreGroup getNew() {
        return new SGSmallStraightYahtzee();
    }
    ...
    public int getScore(int d1, int d2, int d3, int d4, int d5, ArrayList<ScoreGroup> availableMoves) {
        ...
        if(twos>0 && threes>0 && fours>0 && fives>0){
            if(predictedScore>0) {
                ScoreGroup ng = new SGSmallStraightYahtzee();
                ng.predictedScore = fixed;
                ng.description = "Small Straight 2,3,4,5";
                availableMoves.add(ng);
            }else{
                predictedScore = fixed;
                description += "2,3,4,5";
            }
        ...
        if(threes>0 && fours>0 && fives>0 && sixes>0){
            if(predictedScore>0){
                ScoreGroup ng = new SGSmallStraightYahtzee();
                ng.predictedScore = fixed;
                ng.description = "Small Straight 3,4,5,6";
                availableMoves.add(ng);
            }else {
                predictedScore = fixed;
                description += "3,4,5,6";
